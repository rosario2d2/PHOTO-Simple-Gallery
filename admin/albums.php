...
if(!empty($_GET['search'])) {
	...
	if(validate($search, "alnum")) {
		$total_count = Album::count_by_search($session->user_id, $search, array("name"));
		$pagination = new Pagination($page, $per_page, $total_count);
		$offset = $pagination->offset();
		$find = "search=".$search;
		$albums = Album::search_by_word($session->user_id, $search, array("name"), $per_page, $offset);
	} else {
	...
	if(validate($datefrom, "date") && validate($dateto, "date")) {
		$total_count = Album::count_by_date($session->user_id, $datefrom, $dateto);
		$pagination = new Pagination($page, $per_page, $total_count);
		$offset = $pagination->offset();
		$find = "datefrom=".$datefrom."&dateto=".$dateto;
		$albums = Album::search_by_date($session->user_id, $datefrom, $dateto, $per_page, $offset);
	} else {
...
// DEFAULT IF NOT VALID GET OR POST
	...
	$total_count = Album::count($session->user_id);
	$pagination = new Pagination($page, $per_page, $total_count);
	$offset = $pagination->offset();	
	$albums = Album::find($session->user_id, $per_page, $offset);
}
...
?>
		<div id="main">
			<div id="admin-panel">
				<div class="admin-box">
					<img class="h2-icon" src="images/albums_icon.png"><h2 class="left">Albums</h2>
					<div id="admin-box-header">
						<div id="search">
							...
							<form  id="search-by-date" method="get" action="albums.php">
							...
							<form  id="search-by-keyword" method="get" action="albums.php">
					...
					<div class="line"></div>
					<a href="index.php">&laquo; Home</a><a style="float:right;" href="new_album.php">Create a new album</a>
					<div class="info-box">
...
<?php endforeach; ?>
							...
							<tr>
								<td id="total-td" colspan="5">Total Records: <?php echo $total_count; ?></td>
							</tr>
						</table>
					</form>
